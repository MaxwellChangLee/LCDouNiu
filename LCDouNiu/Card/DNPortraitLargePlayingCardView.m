...
@implementation DNPortraitLargePlayingCardView
...
-(void)layoutSubviews
{
    ...
    [self.normalCardImageView setFrame:CGRectMake(self.frame.size.width / 2 - cardWidth / 2, topGap, cardWidth, cardWidth)];
    [self.handStandImageView setFrame:CGRectMake(self.frame.size.width / 2 - cardWidth / 2, self.frame.size.height - topGap - cardWidth, cardWidth, cardWidth)];
}
...
-(UIImageView *)handStandImageView
{
    if (!_handStandImageView) {
        ...
        _handStandImageView.transform = CGAffineTransformMakeRotation(M_PI);
